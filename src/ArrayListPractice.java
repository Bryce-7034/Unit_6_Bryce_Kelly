...
public class ArrayListPractice {
    ...
    public static ArrayList<String> parseIntoArrayList(String egg){
        ArrayList<String> weg = new ArrayList<String>();
        for(int i = 0; i<egg.length(); i++){
            weg.add(egg.substring(i, i+1));
        }
        return weg;
    }
    public static String findMin(ArrayList<Person> egg){
        int ageMax = 9999;
        String name = "";
        for(int i =0; i<egg.size(); i++){
            if(egg.get(i).getAge()<ageMax){
                ageMax = egg.get(i).getAge();
                name = egg.get(i).getMyName();
            }
        }
        return name;
    }
